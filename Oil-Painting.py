...
if __name__ == "__main__":
    default = {
        ...
        "threshold_hsv": (
            ...
            None,
            15,
        ),  # threshold for hsv color space during searching
    ...
    parser.add_argument(
        "--f", type=str, default=default["image"], help="input image path"
    )
    parser.add_argument(
        "--b", type=str, default=default["brush"], help="brush template path"
    )
    parser.add_argument(
        "--p", type=int, default=default["p_max"], help="maximum sampling rate"
    )
    parser.add_argument(
        "--s", type=int, default=default["seed"], help="np.random.seed()"
    )
    parser.add_argument(
        "--force",
        action="store_true",
    ...
    parser.add_argument(
        "--SSAA", type=int, default=default["SSAA"], help="Super-Sampling Anti-Aliasing"
    )
    parser.add_argument(
        "--freq",
        type=int,
    ...
    if 1:
        ...
        time_start = time.time()  # time
        wihte = Gassian_HSV(
            (H0 * SSAA - 2 * padding * SSAA, W0 * SSAA - 2 * padding * SSAA, 3)
        )  # padding
        cv2.imwrite(
            output_path + "/process/{0:04d}.png".format(0),
        ...
        cv2.imwrite(
            output_path + "/Oil_drawing.png", cv2.cvtColor(result, cv2.COLOR_HSV2BGR)
        )
    ...
    if 1:
        ...
        while 1:
            result = cv2.imread(
                output_path + "/Oil_drawing.png", cv2.IMREAD_COLOR
            )  # label输入
            mask_cut = mask[
                max_length * SSAA : -max_length * SSAA,
                max_length * SSAA : -max_length * SSAA,
            ...
            for i in range(centroids.shape[0]):
                ...
                if (
                    p[0] >= padding * SSAA
                    and p[1] >= padding * SSAA
                    and p[0] < result.shape[1] - padding * SSAA
                    and p[1] < result.shape[0] - padding * SSAA
                    and stats[i][4] > 0
                    and stats[i][4] < result.shape[0] * result.shape[1] / 4
                ):
            ...
            if Points.shape[0] == 0:
                ...
                cv2.imwrite(
                    output_path + "/process/Final_Result.png",
                    result[
                        padding * SSAA : -padding * SSAA,
                        padding * SSAA : -padding * SSAA,
                    ],

...
def GetParallel_HSV_new(width, length, period, grayscale):
    ...
    patch = GetParallel(
        distribution=distribution, height=width, length=length, period=period
    )
...
def GetStroke_HSV_pencil(width, length, mean, stroke):
    ...
    canvas = Gassian_HSV(
        size=(width, length, 3), mean=mean, var=[0, 0, 0]
    )  # hsv 颜色空间测试
...
def Getline_Mask_pencil(period, length, var=1):
    ...
    if period % 2 == 1:  # 宽度为奇数
        for i in range(period // 2 + 1):
            mask[i] = Gassian_HSV(
                size=(1, length, 1),
                mean=period / 2 - (period / 2 - i) * 2,
                var=period / 2,
            )
            mask[-i - 1] = Gassian_HSV(
                size=(1, length, 1),
                mean=period / 2 - (period / 2 - i) * 2,
                var=period / 2,
            )
    ...
    for i in range(2 * period):
        ...
        mask[:, i, None] *= np.random.randint(
            low=i + 2 - 2 * period, high=i + 1 + 1, size=(period, 1, 1)
        )
        mask[:, -i - 1, None] *= np.random.randint(
            low=i + 2 - 2 * period, high=i + 1 + 1, size=(period, 1, 1)
        )
...
def Getline_Mask_oil(period, length, var=1):
    ...
    if period % 2 == 1:  # 宽度为奇数
        ...
        for i in range(period // 2):
            ...
            mask[i] = Gassian_Mask(
                size=(1, length, 1), mean=period / 2 - (period / 2 - i) * 2, var=period
            )
            mask[-i - 1] = Gassian_Mask(
                size=(1, length, 1), mean=period / 2 - (period / 2 - i) * 2, var=period
            )
    ...
    for i in range(1 * period):
        #  p = (i+1) / (2*period)
        # numpy.random.randint(low = -i, high=2*period, size=(period, 1, 1))
        mask[:, i, None] *= np.random.uniform(
            low=i + 1 - 1 * period, high=i + 1, size=(period, 1, 1)
        )
        mask[:, -i - 1, None] *= np.random.uniform(
            low=i + 1 - 1 * period, high=i + 1, size=(period, 1, 1)
        )
...
def Getline(distribution, length):
    ...
    if length < constant_length:  # if length is too short, lines are Aligned
        ...
        for i in range(period):
            patch[i] = Gassian(
                (1, length), mean=distribution[i, 0], var=distribution[i, 1]
            )
        ...
        for i in range(period):
            patch[i, begin:-end] = Gassian(
                (1, real_length), mean=distribution[i, 0], var=distribution[i, 1]
            )
...
def Distortion(patch, begin, end):
    ...
    for i in range(length):
        ...
        for j in range(height):
            if j > int_offset:
                patch[j, i] = int(
                    decimal_offset * patch_copy[j - 1 - int_offset, i]
                    + (1 - decimal_offset) * patch_copy[j - int_offset, i]
                )
            else:
...
if __name__ == "__main__":
    ...
    cv2.waitKey(0)
    cv2.imwrite(
        "./input/Brush-oil/" + "color-0.png", cv2.cvtColor(white, cv2.COLOR_HSV2BGR)
    )
    ...
    # cv2 默认BGR
    canvas = Gassian_HSV(
        size=(500, 500, 3), mean=[0, 255, 50], var=[0, 10, 0]
    )  # hsv 颜色空间测试
    canvas = cv2.cvtColor(canvas, cv2.COLOR_HSV2BGR)

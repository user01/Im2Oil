...
def FindPath_V1(
    graymap, angle, y, x, step_length=1, min_length=20, max_length=60, threshold=30
):
    ...
    while 1:
        if Flag_positive == True:
            positive_angle = (
                angle[IntRound(positive_y), IntRound(positive_x)] / 180 * math.pi
            )
        ...
        if Flag_negative == True:
            negative_angle = (
                angle[IntRound(negative_y), IntRound(negative_x)] / 180 * math.pi
            )
        ...
        if (
            positive_y < 0
            or IntRound(positive_y) > H - 1
            or IntRound(positive_x) > W - 1
            or positive_x < 0
        ):
            Flag_positive = False
        if (
            negative_y < 0
            or IntRound(negative_y) > H - 1
            or IntRound(negative_x) > W - 1
            or negative_x < 0
        ):
            Flag_negative = False
...
# 圆心位置是亚像素的      ETF方向不用亚像素，否则太复杂了
def FindPath_V2(
    graymap, angle, y, x, step_length=1, min_length=40, max_length=80, threshold=25
):
    (H, W) = graymap.shape  # image shape
    path = [(y, x)]  # path: a list to save points' trace [y,x]
...
# 灰度和起点的差的绝对值小于阈值
def FindPath_V3(
    graymap, angle, y, x, step_length=1, min_length=10, max_length=80, threshold=25.5
):
    (H, W) = graymap.shape  # image shape
    path = [[y, x]]  # path: a list to save points' trace [y,x]
...
def FindPath_V4(
    ...
    while 1:
        ...
        if Flag_positive == True:
            ...
            if len(path) < min_length:
                path.append([positive_y, positive_x])
            elif grayscale_positive <= grayscale + threshold and (
                abs(angle_positive - angle_origin) < threshold_angle
                or (180 - abs(angle_positive - angle_origin)) < threshold_angle
            ):
                path.append([positive_y, positive_x])
            else:
                Flag_positive = False
        ...
        if Flag_negative == True:
            ...
            if len(path) < min_length:
                path.insert(0, [negative_y, negative_x])
            elif grayscale_negative <= grayscale + threshold and (
                abs(angle_negative - angle_origin) < threshold_angle
                or (180 - abs(angle_negative - angle_origin)) < threshold_angle
            ):
                path.insert(0, [negative_y, negative_x])
            else:
                Flag_negative = False
...
def FindArea_Rec(
    ...
    def color_condition(HSV, hsv, threshold_hsv):
        ...
        if (
            abs(float(HSV[0]) - float(hsv[0])) <= threshold_hsv[0]
            or abs(float(HSV[0]) + 180 - float(hsv[0])) <= threshold_hsv[0]
            or abs(float(HSV[0]) - 180 - float(hsv[0])) <= threshold_hsv[0]
...
def FindPath_V6Line(
    ...
    while 1:
        ...
        # if out of canvas, stop
        if (
            IntRound(positive_y) < 0
            or IntRound(positive_y) > H - 1
            or IntRound(positive_x) > W - 1
            or IntRound(positive_x) < 0
        ):
            Flag_positive = False
        if (
            IntRound(negative_y) < 0
            or IntRound(negative_y) > H - 1
            or IntRound(negative_x) > W - 1
            or IntRound(negative_x) < 0
        ):
            Flag_negative = False

...
def Convert(angle, h, w, y, x1, x2):
    ...
    return (
        int(np.round(y_begin)),
        int(np.round(x_begin)),
        int(np.round(y_end)),
        int(np.round(x_end)),
    )
...
def rotate(image, angle, scale=1.0, pad_color=255):
    ...
    origin_point = np.array(
        [nW / 2.0 - M[0, 1] * h / 2.0, nH / 2.0 - M[1, 1] * h / 2.0]
    )
    ...
    result = cv2.warpAffine(
        src=image, M=M, dsize=(nW, nH), borderValue=(pad_color, pad_color, pad_color)
    )
...
def drawpatch(canvas, patch_size, angle, scale, location, grayscale):
    ...
    (H, W) = canvas.shape
    pad_canvas = np.zeros(
        (2 * h + canvas.shape[0], 2 * w + canvas.shape[1]), dtype=np.uint8
    )
    pad_canvas[h : h + H, w : w + W] = canvas

    Aligned_point = [
        w + location[0] - origin_point[0],
        h + location[1] - origin_point[1],
    ]

    m = np.minimum(
        imgRotation,
        pad_canvas[
            Aligned_point[1] : Aligned_point[1] + h,
            Aligned_point[0] : Aligned_point[0] + w,
        ],
    )
    pad_canvas[
        Aligned_point[1] : Aligned_point[1] + h, Aligned_point[0] : Aligned_point[0] + w
    ] = m

...
class ETF:
    def __init__(
        ...
        img_normal = cv2.normalize(
            img.astype("float32"), None, 0.0, 1.0, cv2.NORM_MINMAX
        )
    ...
    def Wm(self):
        ...
        (h, w) = self.shape
        x = self.gradient_norm[
            self.kernel_radius : -self.kernel_radius,
            self.kernel_radius : -self.kernel_radius,
        ]
        for i in range(self.kernel_size):
            for j in range(self.kernel_size):
    ...
    def forward(self):
        ...
        for iter_time in range(self.iter_time):
            ...
            x_patch = torch.nn.functional.unfold(
                x_magnitude, (self.kernel_size, self.kernel_size)
            )
            y_patch = torch.nn.functional.unfold(
                y_magnitude, (self.kernel_size, self.kernel_size)
            )
    ...
    def save(self, x, y):
        x = nn.functional.interpolate(
            x.unsqueeze(0).unsqueeze(0), [*self.origin_shape], mode="nearest"
        )
        y = nn.functional.interpolate(
            y.unsqueeze(0).unsqueeze(0), [*self.origin_shape], mode="nearest"
        )
        x = x.squeeze()
        y = y.squeeze()
        x[x == 0] += 1e-12
        ...
        if self.background_dir != None:
            t = self.gradient_magnitude[
                self.kernel_radius : -self.kernel_radius,
                self.kernel_radius : -self.kernel_radius,
            ]
            t = nn.functional.interpolate(
                t.unsqueeze(0).unsqueeze(0), [*self.origin_shape], mode="bilinear"
            )
            t = t.squeeze()
            a = t.min()
            b = t.max()
        ...
        for i in range(self.dir_num):
            if i == 0:
                ...
                maximum = -90 + length / 2
                mask1 = 255 * (
                    ((angle > minimum) + (angle == minimum)) * (angle < maximum)
                )
                maximum = 90
                minimum = 90 - length / 2
                ...
                mask = mask1 + mask2
                cv2.imwrite(
                    self.output_path + "/dir_mask{}.png".format(i),
                    np.uint8(mask.numpy()),
                )
            else:
